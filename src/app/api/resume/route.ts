...
function safeText(doc: PdfDoc, text: string, options: SafeTextOptions = {}) {
  ...
  const paragraphGap =
    typeof options.paragraphGap === 'number' ? options.paragraphGap : 0;
...
function renderTemplate1(doc: PdfDoc) {
  ...
  doc.moveDown(0.3);
  doc.font('Helvetica').fontSize(10).fillColor('#111111');
  safeText(doc, `Architecture: ${flagshipProject.architecture}`, {
    lineGap: 2,
  });
  doc.moveDown(0.3);
...
async function buildDocxBuffer(template: string) {
  ...
  children.push(
    normal(`${person.location} | ${person.phone} | ${person.email}`)
  );
  ...
  if (template === '2') {
    ...
    for (const exp of experience) {
      children.push(
        new Paragraph({
          children: [
            new TextRun({ text: `${exp.company} — ${exp.title}`, bold: true }),
          ],
          spacing: { after: 60 },
        })
      );
    ...
    for (const project of featuredProjects.slice(0, 2)) {
      ...
      if ('date' in project && project.date)
        children.push(normal(project.date));
      children.push(normal(project.description));
      if (project.tech?.length)
        children.push(normal(`Tech: ${project.tech.join(', ')}`));
    }
    ...
    for (const p of additionalProjects.slice(0, 8))
      children.push(bullet(`${p.name} — ${p.description}`));
