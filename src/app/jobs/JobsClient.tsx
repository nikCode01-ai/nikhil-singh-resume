...
function normalizeTitle(raw: string) {
  ...
  const withoutPrefix = t.replace(/^\s*(?:hiring|we\s+are\s+hiring)\s*[:\-–—]?\s*/i, "");

  const separators = [" | ", " - ", " – ", " — "];
  for (const sep of separators) {
    if (!withoutPrefix.includes(sep)) continue;
    const parts = withoutPrefix.split(sep).map((p) => p.trim()).filter(Boolean);
    if (parts.length <= 1) continue;

    const first = parts[0];
    const last = parts[parts.length - 1];
    const lastLooksLikeSite = /\b(?:com|in|net|org)\b/i.test(last) || /\./.test(last);
...
function extractJobTitleFromSnippet(snippet: string) {
  ...
  const m = text.match(/\bJob\s*Title\s*:?\s*\*?\s*([^.|•]+?)(?:\s*[.|•]|\s*$)/i);
...
function normalizeSnippet(raw: string) {
  ...
  text = text.replace(/^\s*\d{1,3}\s+(?:minute|minutes|hour|hours|day|days|week|weeks|month|months)\s+ago\s*(?:\.{3}|[•·-])\s*/i, "");
...
function extractSalaryLpa(input: string) {
  ...
  const range = text.match(/\b(\d{1,2}(?:\.\d+)?)\s*[-–—to]{1,3}\s*(\d{1,2}(?:\.\d+)?)\s*(lpa|lakhs|lacs)\b/i);
  if (range) {
...
export function JobsClient() {
  ...
  const buildQueryWithFilters = useCallback(
    (base: string) => {
      ...
      const combinedDomains = Array.from(new Set([...sourceDomains, normalizedCustomDomain].filter(Boolean)));
      ...
      const experienceTerm = experienceYears > 0 ? `${experienceYears}+ years` : "";
      const companyTerm = company.trim();

      return [base, ...workModeTerms, experienceTerm, ...departments, companyTerm, domainsPart]
        .map((v) => v.trim())
        .filter(Boolean)
        .join(" ");
    },
    [company, customDomain, departments, experienceYears, sourceDomains, workModes],
  );
  ...
  const fetchJobs = async ({
    ...
    const res = await fetch(`/api/jobs?${params.toString()}`, { cache: "no-store", signal });
    if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      fetchJobs({ q: qWithFilters, role, location, days, start, num: pageSize, signal: controller.signal }).catch(
        (e: unknown) => {
  ...
  const activeDomains = useMemo(() => {
    const normalizedCustomDomain = normalizeDomainInput(customDomain);
    return Array.from(new Set([...sourceDomains, normalizedCustomDomain].filter(Boolean)));
              ...
              <Card className="p-4">
                ...
                <div className="mt-5 space-y-6">
                  <div className="space-y-3">
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Work mode</div>
                    <div className="space-y-2">
                      {(["Work from office", "Hybrid", "Remote"] as const).map((mode) => {
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              onChange={(e) => {
                                setWorkModes((prev) => {
                                  if (e.target.checked) return [...prev, mode];
                  ...
                  <div className="space-y-3">
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Source</div>
                    <div className="space-y-2">
                      {sourceOptions.map((s) => {
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                setSourceDomains((prev) => {
                                  if (e.target.checked) return [...prev, s.domain];
                    ...
                    <label className="block space-y-1">
                      <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Custom domain</div>
                      <input
                        value={customDomain}
                        onChange={(e) => {
                  ...
                  <div className="space-y-3">
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Company</div>
                    <div className="relative">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Min salary (LPA)</div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Experience</div>
                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">{experienceLabel}</span>
                    </div>
                    <input
                      type="range"
                      min={0}
                  ...
                  <div className="space-y-3">
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Department</div>
                  ...
                  <div className="space-y-3">
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Date posted</div>
                    <select
                      value={days}
                      onChange={(e) => {
              ...
              <Card>
                <form
                  ...
                  onSubmit={(e) => {
                    e.preventDefault();
                    const qWithFilters = buildQueryWithFilters(q);
                    fetchJobs({ q: qWithFilters, role, location, days, start, num: pageSize }).catch((err: unknown) => {
                      setStatus("error");
          ...
          >
            {status === "success" && data ? (
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="text-xs text-slate-500 dark:text-slate-400">Page {currentPage}</div>
                <div className="flex items-center gap-2">
                  <Button
                    type="button"
                    variant="icon"
                  ...
                  <div key={group.label} className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {group.items.map((item) => {
                        const extractedTitle = item.snippet ? extractJobTitleFromSnippet(item.snippet) : null;
                        const finalTitle = normalizeTitle(extractedTitle || item.title || item.link);

                        const extractedLocation = item.snippet ? extractLocationFromSnippet(item.snippet) : null;

                        let cleanedSnippet = item.snippet ? normalizeSnippet(item.snippet) : "";
                        ...
                        const isSaved = Boolean(item.link) && savedLinks.includes(item.link);
                            ...
                            <div className="space-y-3">
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  ...
                                  {item.displayLink ? (
                                    <div className="mt-1 text-xs text-slate-600 dark:text-slate-400">{item.displayLink}</div>
                                  ) : null}
                                </div>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (!item.link) return;
                                    setSavedLinks((prev) => {
                                      if (prev.includes(item.link)) return prev.filter((l) => l !== item.link);
                              ...
                              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div className="flex flex-wrap gap-2">
                                  {extractedLocation ? <Badge>{extractedLocation}</Badge> : null}
                                  {item.date ? (
              ...
              <Card className="p-4">
                ...
                <div className="mt-3 space-y-3">
                  <div className="flex items-center justify-between text-sm text-slate-700 dark:text-slate-300">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{savedLinks.length}</span>
                  </div>

                  <div className="text-xs font-semibold text-slate-700 dark:text-slate-200">Active filters</div>
                  <div className="flex flex-wrap gap-2">
                    {location ? (
                      <Badge className="gap-1">
                    ...
                    {experienceYears > 0 ? <Badge>{experienceLabel}</Badge> : null}
                    {departments.map((d) => (
                      <Badge key={d}>{d}</Badge>
                    ))}
                    {activeDomains.map((d) => (
                    ...
                    {minSalaryLpa > 0 ? <Badge>{minSalaryLpa}+ LPA</Badge> : null}
                  </div>
                </div>
              </Card>
            </aside>
